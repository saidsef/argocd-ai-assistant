...
Every field is optional. A backend serving one model needs no settings at all: `baseURL` falls back to the Argo CD proxy path, and an unset `model` comes from the backend's `/v1/models`. `baseURL`, `apiKey`, and `mcpServers` live under the `data` object, the rest are top-level. The [Settings Extension](../deployment/settings.md#create-the-settings-file) page has the full field reference and how to deploy these settings.
...
- **Let the backend name the model** - leave `model` out. It is looked up once per session, and a backend serving several asks you to pick one of them by name.
