...
| `data.mcpServers` | No | Array of MCP server HTTP endpoints (e.g. `["https://mcp.example.com"]`), each CORS-enabled for the Argo CD origin. The assistant always knows which servers are configured, but only offers a server's tools when the user names it in their message or the one before it. See [MCP Tool Integration](../architecture.md#mcp-tool-integration). |
